...
# METAINFO_PATH=$(grep "metainfo.csv" archive_contents.txt | awk '{print $6}')
# REFERENCE_CURVES_PATH=$(grep "reference_curves/" archive_contents.txt | awk '{print $6}' | head -n 1)

# # Check if the specific files were found
# if [ -z "$METAINFO_PATH" ] || [ -z "$REFERENCE_CURVES_PATH" ]; then
#   echo "Required files not found in the archive."
#   exit 1
# fi

# echo "Found metainfo.csv at: $METAINFO_PATH"
# echo "Found reference_curves/ at: $REFERENCE_CURVES_PATH"
...
tar -xvf "$ARCHIVE_NAME" -C "$DEST_DIR" "$BASE_NAME/metainfo.csv" "$BASE_NAME/reference_curves/"
