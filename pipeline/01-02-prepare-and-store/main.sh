...
export STEP_NAME="01-02-prepare-and-store"
...
INPUT_QUEUE_NAME="MQ1_1_$DRY_RUN_ID"
OUTPUT_QUEUE_NAME="MQ1_2_$DRY_RUN_ID"
...
if [ -z "$SOURCE_DIR" ] || [ "$SOURCE" == "null" ] || [ "$DATA" == "null" ]; then
...
# CODE TO EXECUTE
python3 prepare.py $LOCAL_WORK_DIR
...
export INPUT_DATA_VOLUME=$(du -cb "$LOCAL_WORK_DIR" "$LOCAL_WORK_DIR" | grep total | awk '{print $1}')
...
echo "DATA_TRANSMISSION_TIME: $DATA_TRANSMISSION_TIME milliseconds"
...
MESSAGE="{\"DATA\": \"n/a\", \"SOURCE\": \"$RESOURCE_NAME\"}"
echo "Sending message to RabbitMQ queue $OUTPUT_QUEUE_NAME..."
rabbitmqadmin publish routing_key=$OUTPUT_QUEUE_NAME payload="$MESSAGE" \
  --username="$RABBITMQ_USER" --password="$RABBITMQ_PASS" --host="$RABBITMQ_HOST" --port="$RABBITMQ_PORT"
